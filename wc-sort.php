...
final class Sort {
	...
	final public function init(): void {
		...
		$controllerRegistrar->register("woocommerce_catalog_orderby",
		...
		$controllerRegistrar->register("pre_option_woocommerce_default_catalog_orderby",
			SetTrendingOptionAsDefaultController::class);

		$controllerRegistrar->register("woocommerce_get_catalog_ordering_args",
			GetCatalogArgumentsForOrderingController::class,
			11);

		$controllerRegistrar->register("woocommerce_product_duplicate",
		...
		register_deactivation_hook("{$dataHelper->getAppRoot()}/wc-sort.php",
		...
		if(substr($key, 0, strlen(__NAMESPACE__)) !== __NAMESPACE__) {
			add_action("admin_notices",
				function() use ($key): void {
					$engine = new Engine(__DIR__ . "/templates");
					echo $engine->render("error-notice", ["e" => new Exception("Invalid namespace in '$key'")], TRUE);
				},
				-1,
			);
		}
			...
			try {
				...
				add_action("admin_notices",
					function() use ($e): void {
						...
						$engine = new Engine(__DIR__ . "/templates");
						echo $engine->render("error-notice", ["e" => $e], TRUE);
					},
					-1,
				);
			}
